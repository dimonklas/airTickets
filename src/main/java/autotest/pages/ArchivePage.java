...
//        $x(".//*[text()='Оплатить']").shouldBe(exist, enabled);   //временно убрал из-за блокировки после пятой брони
    ...
    public void clickStornBookingButton(String bookingId) {
        ...
        sleep(1000);
        if (!$x(".//*[text()='Основная информация']").isDisplayed())
            $x(".//*[text()='OK']").shouldBe(visible, enabled.because("Кнопка ОК")).click();  //костыль
